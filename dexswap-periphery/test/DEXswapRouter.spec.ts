...
describe('DexSwapRouter', () => {
  ...
  beforeEach(async function() {
    ...
    factory = fixture.dexSwapFactory
  ...
  it('quote', async () => {
    ...
    await expect(router.quote(bigNumberify(0), bigNumberify(100), bigNumberify(200))).to.be.revertedWith(
      'DexSwapLibrary: INSUFFICIENT_AMOUNT'
    )
  ...
  it('getAmountOut', async () => {
    ...
    ).to.be.revertedWith('DexSwapLibrary: INSUFFICIENT_INPUT_AMOUNT')
    await expect(
      router.getAmountOut(bigNumberify(2), bigNumberify(0), bigNumberify(100), bigNumberify(30))
    ).to.be.revertedWith('DexSwapLibrary: INSUFFICIENT_LIQUIDITY')
    await expect(
  ...
  it('getAmountIn', async () => {
    ...
    ).to.be.revertedWith('DexSwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT')
    await expect(
...
describe('fee-on-transfer tokens', () => {
  ...
  beforeEach(async function() {
    ...
    await fixture.dexSwapFactory.createPair(DTT.address, WETH.address)
    const pairAddress = await fixture.dexSwapFactory.getPair(DTT.address, WETH.address)
    pair = new Contract(pairAddress, JSON.stringify(IDexSwapPair.abi), provider).connect(wallet)
  })
...
describe('fee-on-transfer tokens: reloaded', () => {
  ...
  beforeEach(async function() {
    ...
    // make a DTT<>WETH pair
    await fixture.dexSwapFactory.createPair(DTT.address, DTT2.address)
    const pairAddress = await fixture.dexSwapFactory.getPair(DTT.address, DTT2.address)
  })
